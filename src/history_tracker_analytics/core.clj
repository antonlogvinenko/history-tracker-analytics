...
;;(new class) (class.)
;;(.mem obj) (.meth obj arg)
;;(class/mem) (class/meth arg)
;;(. anything mem) (. anything meth arg)
;;(.. anything (meth arg) meth)
;;(doto obj (.meth) (.meth arg))
;;memfn bean
...
;;add if null-s
;search, extracting 100 elements
;;make bigger xmls, run several times, write table function, read, approximage, use
(defn parse-xml [xml]
...
;;TODO
;;generate differnet size xmls considering bytes
;;each several times, then find average time, write to file
;;parse xml, find in the middle, than extract 100 more if available
(defn measure [sample])

;;TODO fix interpolator
...
(defn capacity-to-time-avg [length]
  (let [sample (generate-xml-string length)
    ...
    {:bytes bytes :time (/ generating-time / repeat-count)}))
        

(defn generate-table-function []
  (let [versions (range 1 10000 200)
        capacity-to-time (map capacity-to-time-avg versions)]
    (spit "table.f" capacity-to-time)))
